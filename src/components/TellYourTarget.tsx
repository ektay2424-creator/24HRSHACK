...
import { Target, Loader2, Sparkles, Rocket, CheckCircle2, TrendingDown } from 'lucide-react';
import { Badge, Panel } from './ui';
import { useToast } from './Toast';
import { calculateGoal, type GoalResult } from '@/lib/api';
import { cn, formatINR } from '@/lib/utils';

export function TellYourTarget({ connected }: { connected: boolean }) {
  const toast = useToast();
  const [targetName, setTargetName] = useState('Nike Shoes');
  const [targetCost, setTargetCost] = useState(6000);
  const [costInput, setCostInput] = useState('6000');
  const [monthlySaving] = useState(1713);
  const [goal, setGoal] = useState<GoalResult | null>(null);
  const [loading, setLoading] = useState(false);
  const [animatePct, setAnimatePct] = useState(0);

  useEffect(() => {
    recompute(targetName, targetCost);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const recompute = async (name: string, cost: number) => {
    setLoading(true);
    setAnimatePct(0);
    const { data, live } = await calculateGoal(name, cost, monthlySaving);
    setGoal(data);
    setLoading(false);
    const finalPct = data.timeline.length ? data.timeline[data.timeline.length - 1].pct : 0;
    setTimeout(() => setAnimatePct(finalPct), 120);
    if (live) toast('Goal timeline recalculated with live backend.', 'info');
  };
    ...
    <div className="space-y-6">
      ...
      {!connected && (
        <p className="text-center text-xs text-slate-600">
          Showing projected timeline from cached savings — backend not reachable. Recalculation still runs locally.
        </p>
      )}
    </div>
  );
